...
function addressesDiffer(stored, fetched) {
  ...
  const a = normalizeSido(stored).replace(/ /g, '');
  const b = normalizeSido(fetched).replace(/ /g, '');
  if (a === b) return false;
  ...
  if (!shorter || !longer.startsWith(shorter)) return true;
  return /\d/.test(longer[shorter.length]);
}
