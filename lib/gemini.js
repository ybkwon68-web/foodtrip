...
async function callGemini(prompt) {
  ...
  if (res.status === 503) {
    const err = new Error('죄송합니다. AI 서버가 일시적으로 혼잡합니다. 잠시 후 다시 시도해 주세요.');
...
- 일부 후보에는 broadcast_excerpt(방송 원문 일부)가 붙어 있습니다. 있으면 분위기·특징(조용함, 왁자지껄함, 가족적, 데이트 등) 판단에 적극 참고하세요. 없는 후보는 이름/메뉴/한줄평만으로 판단하세요.
